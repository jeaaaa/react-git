    ...
    return {
        ...
        trigger: function trigger(action, options) {
            ...
            if (options.newData) {
                ...
                if (typeof _config2.data == 'function' && data_conf !== _config2.data) {
                    //action配置时无data 就会继承全局配置的data函数 导致同一函数执行2次
                    data2 = _config2.data.call(target, conf);
                }
            ...
            promise = promise.then(conf._callback).fail(function (e) {
                return conf._callback({ error: e.statusText });
            });
