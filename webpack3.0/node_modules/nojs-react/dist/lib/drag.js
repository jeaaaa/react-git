...
$(document.body).append('<style>.addIndex{z-index:99999}</style>');

var drag = function drag(_drag, move, opt) {
	/*
  * 元素拖拽
  * drag:拖拽对象，jquery对象
  * move:拖动可控制区域，不传则整个拖拽对象都可拖动
  */
	this.drag = _drag; //拖拽对象
	this.opt = opt = opt || {};
	this.move = move ? move : _drag; //拖拽控制区
	...
	this.onDragDown = opt.onDragDown; //鼠标按下触发事件
	this.onDragStart = opt.onDragStart; //开始拖拽事件
	this.MoveEvent = opt.MoveEvent; //鼠标移动触发事件
	this.UpEvent = opt.UpEvent; //鼠标松开触发事件
...
drag.prototype = {
	...
	DragDown: function DragDown(e) {
		...
		if (this.onDragDown && this.onDragDown.call(this, e) == false) {
			return false;
		}
		...
		if (this.onDragStart && this.onDragStart.call(this, e) == false) {
			return false;
		}
	...
	DragUp: function DragUp(e) {
		if (!this.disable) {
			...
			this.UpEvent && this.UpEvent.call(this, e);
